...
export const PROCESS_PANEL_SETTING_OPTIONS: Array<{
...
export const getProcessEventLabel = (event: MessageProcessEvent): string => {
  switch (event.type) {
    case 'status':
      return 'Status';
    case 'log':
...
export const getProcessEventSummary = (event: MessageProcessEvent): string => {
  switch (event.type) {
    case 'status':
      return event.label;
    case 'log':

...
if %errorlevel% neq 0 (
    echo     ngrok 未安装，将跳过隧道功能
    echo     如需外网访问，请手动安装 ngrok
) else (
    :: 检查 ngrok authtoken
    echo [3/5] 检查 ngrok 配置...
    set "NGROK_CONFIG=%USERPROFILE%\AppData\Local\ngrok\ngrok.yml"
    if not exist "%NGROK_CONFIG%" (
        echo.
        echo ========================================
        echo   首次使用需要配置 ngrok
        echo   请访问 https://dashboard.ngrok.com/signup 注册
...
start "CodeRemote HTTP" cmd /k "cd /d "%SCRIPT_DIR%web" && npx http-server -p 8084 -c-1"
...
start "CodeRemote CLI" cmd /k "cd /d "%SCRIPT_DIR%cli" && npx code-remote start --port 8085"

:: 等待 CLI 服务器启动
timeout /t 3 /nobreak >nul

:: 启动 ngrok 隧道 (检查 ngrok 是否可用)
where ngrok >nul 2>nul
if %errorlevel% equ 0 (
    echo.
    echo 启动 ngrok 隧道...
...
if %errorlevel% equ 0 (
    ...
    echo 如需外网访问，请手动运行: ngrok http 8085
)

echo.
echo Token: 请查看 CLI 服务器窗口
echo.
echo 按任意键打开浏览器测试...
pause >nul

...
export class ClaudeCodeEngine {
  ...
  private getClaudeCommand(): string {
    return process.env.CLAUDE_CLI_COMMAND?.trim() || 'claude';
  }

  private getMcpConfigPath(): string {
    return process.env.CODEREMOTE_MCP_CONFIG?.trim()
  ...
  async detectClaudeCLI(): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      const proc = spawn(this.getClaudeCommand(), ['--version'], { shell: true });
  ...
  private async callClaudeCLI(
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log('[Claude CLI] Command args:', claudeCommand, args.join(' '));
